...
var _ = Describe("bilobad", func() {
	...
	It("serves a framed handshake and shuts down when its parent closes stdin", func() {
		...
		Expect(err).NotTo(HaveOccurred())
		Expect(protocol.NewFramedWriter(stdinWriter).Write(protocol.Request{ID: 1, Method: "handshake", Params: params})).To(Succeed())
		var response protocol.Response
		Expect(protocol.NewFramedReader(stdoutReader).Read(&response)).To(Succeed())
